$(function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $.each(data.list, function (index, item) {
                    ...
                    var cid = item.cid;
                    $(".infobox .logdiv .maininfo").append("<div class=\"show clearfix\">\n" +
                        "                                <div class=\"pull-left\">\n" +
                        ...
                        "                                <div class=\"pull-left\">\n" +
                        "                                    <p class=\"show-title\">\n" +
                        "                                        <a href=\"/forward_con/showcourse?id=" + cid + "\" target='_blank'>" + cname + "</a>\n" +
                        "                                    </p>\n" +
                        "                                    <p class=\"show-time\">" + logtime + "</p>\n" +
                        "                                </div>\n" +
                        "                                <a href=\"javascript: ;\" class=\"remove\" data-rid='" + id + "' title=\"点击删除该记录\"><span class=\"glyphicon glyphicon-remove\"></span></a>\n" +
                        "                            </div>");
                });
            },
            error: function (xhr, status) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (status === "invalid") {
                    ...
                    $this.closest("div.show").animate({
                        height: 0
                    },100);
                    window.setTimeout(function(){
                        $this.closest("div.show").remove();
                    },100);
                }
            },
            error: function (xhr, status) {
    ...
    $(document).on("click", ".historydiv .maininfo .show .remove", del_courselog);
    ...
    var async_coursecommand = function(){
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                if(count == 0){
                    ...
                    $.each(data.list, function(index, item){
                        ...
                        var $topp = $("<p class='main-top'>评分星级&nbsp;&nbsp;&nbsp;&nbsp;</p>");
                        while(score-- != 0){
                            $topp.append("<span class='glyphicon glyphicon-star'></span>");
                        }
                       $(".commanddiv .maininfo").append("<div class=\"show\">\n" +
                           "                            <div class=\"pull-left\">\n" +
                           "                                <a href=\"/forward_con/showcourse?id="+cid+"\" target='_blank'>\n" +
                           "                                    <img src=\""+imgsrc+"\"/>\n" +
                           "                                </a>\n" +
            ...
            error: function(xhr, status){
                window.alert("后台环境异常导致无法获取我的课程评论数据，请稍后再试");
